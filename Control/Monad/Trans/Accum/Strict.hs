...
module Control.Monad.Trans.Accum.Strict (
    -- * The Accum monad
    Accum,
    accum,
    runAccum,
...
-- | @'tell' w@ is an action that monoidally appends @w@ to the accumulated output.
tell :: (Monoid w, Monad m) => w -> AccumT w m ()
tell w = AccumT $ \ w' -> return ((),mappend w' w)
{-# INLINE tell #-}

-- | @'tell\'' w@ is an action that monoidally appends @w@ to the accumulated output,
-- evaluating the acculumated output to WHNF.
tell' :: (Monoid w, Monad m) => w -> AccumT w m ()
tell' w = AccumT $ \ w' ->
  let x = mappend w' w
   in x `seq` return ((),x)
